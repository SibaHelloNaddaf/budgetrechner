...
describe('budgetCalculations', () => {
  ...
  it('liefert einen negativen saldo bei höheren ausgaben', () => {
    ...
    expect(calculateBalance(transactionsWithLoss)).toBe(-200)
  })

  it('akzeptiert eine gültige income-transaktion', () => {
    expect(
      validateTransaction({ description: 'Gehalt', amount: 2500, type: 'income' }),
    ).toBe(true)
  })

  it('akzeptiert eine gültige expense-transaktion', () => {
    expect(
      validateTransaction({ description: 'Miete', amount: 900, type: 'expense' }),
    ).toBe(true)
  })

  it('lehnt eine leere oder nur aus leerzeichen bestehende beschreibung ab', () => {
    expect(validateTransaction({ description: '   ', amount: 2500, type: 'income' })).toBe(false)
  })

  it('lehnt ungültige betragwerte wie NaN, Infinity, <= 0 oder string ab', () => {
    expect(validateTransaction({ description: 'Gehalt', amount: NaN, type: 'income' })).toBe(false)
    expect(validateTransaction({ description: 'Gehalt', amount: Infinity, type: 'income' })).toBe(false)
    expect(validateTransaction({ description: 'Gehalt', amount: 0, type: 'income' })).toBe(false)
    expect(validateTransaction({ description: 'Gehalt', amount: -50, type: 'income' })).toBe(false)
    expect(validateTransaction({ description: 'Gehalt', amount: '2500', type: 'income' })).toBe(false)
  })

  it('lehnt einen ungültigen typ ab', () => {
    expect(validateTransaction({ description: 'Gehalt', amount: 2500, type: 'transfer' })).toBe(false)
  })
})
